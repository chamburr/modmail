...
class Configuration(commands.Cog):
    ...
    async def setup(self, ctx):
        ...
        try:
            ...
            access_role = await commands.RoleConverter().convert(
                ctx, access_role.content
            )
            if not access_role:
                return await ctx.send(
                    embed=discord.Embed(
        ...
        c.execute(
            "UPDATE data SET category=?, accessrole=?, logging=?, pingrole=? WHERE guild=?",
            (
                category.id,
                access_role.id,
                logging_channel.id if logging_channel is not None else None,
                str(ping_role.id)
                if ping_role is not None and ping_role not in ["@here", "@everyone"]
                else ping_role,
                ctx.guild.id,
            ),
        )
    ...
    async def category(self, ctx, *, name: str = "ModMail"):
        ...
        c = self.bot.conn.cursor()
        c.execute(
            "UPDATE data SET category=? WHERE guild=?", (category.id, ctx.guild.id)
        )
        self.bot.conn.commit()
        await ctx.send(
            embed=discord.Embed(
    ...
    async def logs(self, ctx):
        ...
        if data[4] is not None:
            ...
            c = self.bot.conn.cursor()
            c.execute(
                "UPDATE data SET logging=? WHERE guild=?", (channel.id, ctx.guild.id)
            )
            self.bot.conn.commit()
            await ctx.send(
                embed=discord.Embed(
    ...
    async def loggingplus(self, ctx):
        ...
        if data[7] is None:
            ...
            c.execute(
                "UPDATE data SET loggingplus=? WHERE guild=?", (None, ctx.guild.id)
            )
            await ctx.send(
                embed=discord.Embed(
